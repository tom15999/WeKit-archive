...
// Returns true if the line represents a file-backed mapping with a non-empty
// path.
static bool parse_maps_line(const char *line, MapEntry *out)
{
    ...
    int n = sscanf(line, "%llx-%llx %7s %llx %x:%x %lu %511s", &start, &end,
                   perms, &offset, &dev_maj, &dev_min, &inode, path);

    if (n < 7)
        return false;
    if (path[0] == '\0' || path[0] == '[')
        return false; // anonymous / pseudo
...
static bool remap_segment(const MapEntry *e)
{
    ...
    {
        addr = mmap(reinterpret_cast<void *>(e->start), len, orig_prot,
    ...
    {
        addr = mmap(reinterpret_cast<void *>(e->start), len, PROT_READ | PROT_WRITE,
                    MAP_PRIVATE | MAP_FIXED, mfd, 0);
    }
...
int so_hide_path(const char *needle)
{
    ...
    while (fgets(line, sizeof(line), maps) && entries.size() < MAX_ENTRIES)
    {
        ...
        if (!parse_maps_line(line, &e))
            continue;
        if (strstr(e.path, needle) == nullptr)
            continue;
        entries.push_back(e);
    }
    fclose(maps);
